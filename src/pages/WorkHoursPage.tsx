...
const BREAK_START = '[BREAK_SETTINGS]';
const BREAK_END = '[/BREAK_SETTINGS]';
...
export function WorkHoursPage() {
  ...
  function cleanNotes(notes: string | null) {
    ...
    return notes
      .replace(/\[BREAK_SETTINGS\][\s\S]*?\[\/BREAK_SETTINGS\]/g, '')
      .trim();
  }

  function buildBreakBlock(breakDeducted: boolean) {
    return [
      BREAK_START,
      `break_deducted=${breakDeducted ? 'true' : 'false'}`,
      BREAK_END,
    ].join('\n');
  }

  function parseBreakDeducted(entry: WorkHour) {
    const directValue = (entry as any).break_deducted;
    ...
    const match = notes.match(/break_deducted=(true|false)/i);
  ...
  async function loadData() {
    ...
    if (hoursRes.error) setErrorMessage(hoursRes.error.message);
  ...
  function calculateHours() {
    ...
    const deductedBreakMinutes = formData.break_deducted ? formData.break_minutes : 0;
    const totalMinutes = shiftMinutes - deductedBreakMinutes;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const breakBlock = buildBreakBlock(formData.break_deducted);
    const notesWithBreakSettings = [visibleNotes, breakBlock].filter(Boolean).join('\n\n');
    ...
    const record = {
      ...
      notes: notesWithBreakSettings || null,
    };
  ...
  function editEntry(entry: WorkHour) {
    setFormData({
      ...
      break_deducted: parseBreakDeducted(entry),
    ...
    <div>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setFormData(f => ({ ...f, work_date: e.target.value }))
                  }
                  required
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setFormData(f => ({ ...f, job_site_id: e.target.value }))
                  }
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setFormData(f => ({ ...f, end_time: e.target.value }))
                    }
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <label className="mt-2 flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                    <input
                      type="checkbox"
                      checked={formData.break_deducted}
                      onChange={e =>
                        setFormData(f => ({
                          ...f,
                          break_deducted: e.target.checked,
                        }))
                      }
                      className="rounded border-gray-300 text-teal-600 focus:ring-teal-500"
                    />
                    Deduct break from billable hours
                  </label>

                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {formData.break_deducted
                      ? 'Break is unpaid and deducted.'
                      : 'Break is included/paid.'}
                  </p>
                </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 text-sm">
                ...
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                  <span>Subtotal:</span>
                  <span className="font-medium">
                    ${(calculateHours() * formData.hourly_rate).toFixed(2)}
                  </span>
                </div>

                {profile?.gst_enabled && (
                  <div className="flex justify-between text-gray-600 dark:text-gray-400">
                    <span>GST ({((profile.gst_rate || 0.05) * 100).toFixed(0)}%):</span>
                    <span className="font-medium">
                      $
                      {(
                        calculateHours() *
                        formData.hourly_rate *
                        (profile.gst_rate || 0.05)
                      ).toFixed(2)}
                    </span>
                  </div>
                )}
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          ...
          filtered.map(entry => {
            const breakDeducted = parseBreakDeducted(entry);

            return (
              <div
                key={entry.id}
                className="bg-white dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 p-3 hover:border-teal-300 dark:hover:border-teal-700 transition-colors"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                      {(entry as any).clients?.name || 'No client'}
                      {(entry as any).job_sites?.site_name
                        ? ` • ${(entry as any).job_sites.site_name}`
                        : ''}
                    </p>
                    ...
                    {entry.break_minutes > 0 && (
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        ...
                        {breakDeducted ? 'deducted' : 'included/paid'}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className={`p-1.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors ${
                        entry.status === 'invoiced' || entry.status === 'paid'
      ...
      {deleteTarget && (
        <ConfirmDeleteModal
          ...
          itemName={`${format(
            new Date(deleteTarget.work_date + 'T00:00'),
            'EEE, MMM d'
          )} - ${deleteTarget.total_hours?.toFixed(1)}h`}
          usageInfo={
